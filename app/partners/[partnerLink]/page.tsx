...
const PartnerLinkPage = async({ params }: { params: { partnerLink: string } }) => {
  ...
  return (
    <Panel className={`h-fit w-full relative`}>
      <Header 
            ...
            {!serviceList ? (
              ...
              <>
                {serviceList.map((service: any, index: number) => (
                  ...
                  >
                    <h2 className={`pt-[20px] pb-[5px] leading-none text-[#27ae60] lg:text-[19.6px] font-bold order-[${index}]`}>{service.title}</h2>
                    {service.services.map((subservice: any, subIndex: number) => (
                      <Panel key={subIndex} className="flex flex-col leading-relaxed w-fit text-sm lg:text-[15.2px] text-[#3f51b5]">
                        <Link
                          prefetch={false}
